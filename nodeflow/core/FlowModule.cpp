...
namespace nf
{
	...
	const std::set<std::string>& FlowModule::categories() const
	{
	...
	std::set<std::string> FlowModule::registered() const
	{
		...
		std::transform(m_flowNodeCreators.begin(), m_flowNodeCreators.end(),
			std::inserter(ret, ret.end()),
			[](auto pair) { return pair.first; });
		std::transform(m_dataNodeCreators.begin(), m_dataNodeCreators.end(),
			std::inserter(ret, ret.end()),
			[](auto pair) { return pair.first; });
	...
	nf::Expected<std::pair<std::string, std::string>, nf::RegisterError> FlowModule::helperParseNamePath(const std::string& namePath) const
	{
		if (namePath.empty())
