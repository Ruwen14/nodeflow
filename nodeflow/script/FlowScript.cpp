...
namespace nf
{
	...
	nf::Node* FlowScript::findNode(NodeHandle uuid) const
	{
		...
		//        But I'm not sure yet how to execute the nodes later. Vectors might be more performant then.
	...
	bool FlowScript::removeNode(NodeHandle node)
	{
		...
		foundNode->breakAllConnections(PortDirection::Both);
	...
	*/
	...
	Expected<void, ConnectionError> FlowScript::connectNodes(NodeHandle outNodeUUID, PortIndex outPort, 
	...
	bool FlowScript::disconnectNodes(NodeHandle outNodeUUID, PortIndex outPort, NodeHandle inNodeUUID, PortIndex inPort)
	{
		auto outNode = findNode(outNodeUUID);
		auto inNode = findNode(inNodeUUID);
	...
	*/
	...
	std::string FlowScript::nodeOutputAsStr(NodeHandle node, PortIndex index)
	{
		auto foundNode = findNode(node);
		NF_ASSERT(foundNode != nullptr, "Node doesn't exist");
		NF_ASSERT(index >= 0, "Invalid port index");
		std::stringstream stream;
		auto success = foundNode->streamOutput(index, StreamFlag::WriteTo, stream);
		NF_ASSERT(success, "Node did not implement streamOutput(...");
		return stream.str();
	}

	void FlowScript::setNodeOutputFromStr(NodeHandle node, PortIndex index, const std::string& str)
	{
		auto foundNode = findNode(node);
		NF_ASSERT(foundNode != nullptr, "Node doesn't exist");
		NF_ASSERT(index >= 0, "Invalid port index");
		std::stringstream stream(str);
		auto success = foundNode->streamOutput(index, StreamFlag::ReadFrom, stream);
		NF_ASSERT(success, "Node did not implement streamOutput(...");
	}

	StartEventNode& FlowScript::startEventNode() const
	{
		return *m_startEventNode;
	}
