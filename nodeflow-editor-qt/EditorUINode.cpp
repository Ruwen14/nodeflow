...
namespace nf
{
void DefaultNodePainter::paintNodeCaption(QPainter* painter,
                                          const QRectF& geometry,
                                          const EditorUINode& node)
{
    ...
    path.arcTo(
        x + width - 2 * roundness, y, 2 * roundness, 2 * roundness, 0, 90);
    path.closeSubpath();
    ...
    painter->drawText(geometry.left() + 0.1 * geometry.width(),
                      geometry.center().y(),
                      "Example Node");
    font.setItalic(true);
    font.setBold(false);
    ...
    painter->drawText(geometry.left() + 0.1 * geometry.width(),
                      geometry.center().y() + font.pointSize() * 1.4,
                      "Math");

    // Draw cursive 'f' icon symbol
    painter->setFont(cursiveFFont);
    painter->setPen(QColor(98, 162, 206));
    painter->drawText(geometry.top() + 0.04 * geometry.width(),
                      geometry.center().y() + cursiveFFont.pointSize() / 3,
                      "f");
}

void DefaultNodePainter::paintNodeBody(QPainter* painter,
                                       const QRectF& geometry,
                                       const EditorUINode& node)
{
...
void DefaultNodePainter::paintSelected(QPainter* painter,
                                       const QRectF& geometry,
                                       const EditorUINode& node)
{
    const auto roundness = 10;
...
void DefaultNodePainter::paintError(QPainter* painter,
                                    const QRectF& geometry,
                                    const EditorUINode& node)
{
    const auto roundness = 10;
    ...
    warningIcon.paint(painter,
                      geometry.toRect().adjusted(10, 10, -10, 0),
...
void DefaultPortPainter::paintDataPort(QPainter* painter,
                                       QRectF& geom,
                                       ...
                                       const QColor& color)
{
    ...
    static QRectF circleBBox(geom.x() + 3,
                             geom.center().y() / 3.,
                             2 * circleRadius,
                             2 * circleRadius);
    static QPointF triangle[3] = {
        QPointF(circleBBox.left() + 1.6 * circleRadius,
                circleBBox.center().y() - 0.85 * circleRadius),
        QPointF(circleBBox.left() + 2.6 * circleRadius,
                circleBBox.center().y()),
        QPointF(circleBBox.left() + 1.6 * circleRadius,
                circleBBox.center().y() + 0.85 * circleRadius)
    };
    ...
    if (connected)
    {
        painter->setBrush(color);
        painter->drawEllipse(
            circleBBox.center(), circleRadius / 2, circleRadius / 2);
    }
    ...
    painter->setPen(Qt::lightGray);
    painter->drawText(
        geom.adjusted(4 * circleRadius, 0, 0, 0), Qt::AlignVCenter, caption);
...
void DefaultPortPainter::paintFlowPort(QPainter* painter,
                                       QRectF& geom,
...
UINodeDataPort::UINodeDataPort(std::shared_ptr<PortPainterDelegate> portPainter,
                               QGraphicsItem* parent /*= nullptr*/)
    : QGraphicsItem(parent)
    , m_portGeometry(0, 0, 30, 30)
    , m_portPainter(portPainter)
{
    m_caption = QList<QString>({ "Attack Time",
                                 "Decay Time",
                                 "Count",
                                 "Value",
                                 "Index",
                                 "Array" })[qrand() % 5];
    m_portGeometry.setWidth(
        m_portGeometry.width()
        + 2 * QFontMetrics(QApplication::font()).width(m_caption));

    m_color = QList<QColor>({ QColor(153, 193, 115),
                              QColor(149, 59, 75),
                              QColor(86, 165, 147) })[qrand() % 3];
...
void UINodeDataPort::paint(QPainter* painter,
                           const QStyleOptionGraphicsItem* option,
                           QWidget* widget /* = nullptr */)
{
    m_portPainter->paintDataPort(
        painter, m_portGeometry, *this, m_caption, bool(qrand() % 2), m_color);
...
UINodeFlowPort::UINodeFlowPort(std::shared_ptr<PortPainterDelegate> portPainter,
                               QGraphicsItem* parent /*= nullptr*/)
    : QGraphicsItem(parent)
    , m_portGeometry(0, 0, 30, 30)
    , m_portPainter(portPainter)
...
void UINodeFlowPort::paint(QPainter* painter,
                           const QStyleOptionGraphicsItem* option,
                           QWidget* widget /* = nullptr */)
{
    m_portPainter->paintFlowPort(
        painter, m_portGeometry, *this, bool(qrand() % 2));
...
EditorUINode::EditorUINode(std::unique_ptr<NodePainterDelegate> nodePainter,
                           std::shared_ptr<PortPainterDelegate> portPainter,
                           QGraphicsObject* parent /*= nullptr*/)
    : QGraphicsObject(parent)
    , m_nodeGemoetry(0, 0, 400, 250)
    , m_nodePainter(std::move(nodePainter))
    , m_portPainter(portPainter)
{
    setFlag(QGraphicsItem::ItemIsSelectable);
...
void EditorUINode::setNodePainterDelegate(
    std::unique_ptr<NodePainterDelegate> painter)
{
    m_nodePainter = std::move(painter);
}

void EditorUINode::setPortPainterDelegate(
    std::shared_ptr<PortPainterDelegate> painter)
{
...
void EditorUINode::paint(QPainter* painter,
                         const QStyleOptionGraphicsItem* option,
                         QWidget* widget /* = nullptr */)
{
    ...
    painter->restore();
    painter->save();
    m_nodePainter->paintNodeCaption(painter,
                                    QRectF(m_nodeGemoetry.x(),
                                           m_nodeGemoetry.y(),
                                           m_nodeGemoetry.width(),
                                           m_nodeGemoetry.height() * 0.2),
                                    *this);
    painter->restore();
