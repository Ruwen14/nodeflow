...
static QIcon createPlusIcon()
{
    ...
    painter.drawLine(QPoint(0, mask.rect().height() / 2),
                     QPoint(mask.rect().width(), mask.rect().height() / 2));
    painter.drawLine(QPoint(mask.rect().width() / 2, 0),
                     QPoint(mask.rect().width() / 2, mask.rect().height()));
    pixmap.setMask(mask);
...
CollapsableSection::CollapsableSection(const QString& title,
...
VariableAddConfigurator::VariableAddConfigurator(
    ...
    , m_varDefaultValueEdit(new QLineEdit)
{
    ...
    box->setStyleSheet("QGroupBox::title {color: darkGray;} "
                       "QGroupBox{background-color:rgb(61, 61, 60);}");
    auto font = QApplication::font();
    font.setBold(true);
    font.setPointSize(8);
    box->setFont(font);
...
ScriptVariablesTable::ScriptVariablesTable(
    ...
    , m_addVarPopup(new AddVariableConfigPopup(availabelVarTypes, this))
    , m_availVarTypes(availabelVarTypes)
{
    auto lay = new QVBoxLayout(this);
    ...
    connect(m_addVarButton,
            &QPushButton::clicked,
            this,
            &ScriptVariablesTable::spawnAddVariableOptionDialog);
...
bool ScriptVariablesTable::addNewVariable(const QString& varName,
...
void ScriptVariablesTable::spawnAddVariableOptionDialog()
{
    ...
    // 		{
    // 			varTypeSelector->addItem(createVariableTicTacIcon(Qt::red),
    // varType); 			qDebug() << varType;
    // 		}
    //
    ...
    : QWidget(parent)
{
    ...
    auto scriptVariablesSection =
        new nf::CollapsableSection("VARIABLES", scriptVarablesSectionLayout);
